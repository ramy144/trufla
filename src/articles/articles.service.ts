...
export class ArticleService{
    ...
    private async _checkDuplication(title:string):Promise<void>{
        ...
        if(article){
            throw new ConflictException("there is another article with the same name")
        }
    }

    async findArticleById(articleId:string):Promise<ArticleModel>{
        const article = await this._articlesRepo.model.findById(articleId).populate('author')
        if(!article){
    ...
    async findUsersPaginated(
        ...
        const articles = await this._articlesRepo.findPaginated(query, page, limit,{_id:-1});
